...
All five strategies' Phase-0/Gate-0/Step-−1 tasks, merged and deduplicated (§5 sprint plan). Cheap (days each), and every science claim in the portfolio hangs on them. Includes the time-critical operational items *(2026-08-17 update: Winer is offline for monsoon season until October 2026, so these are now submit-now / execute-at-re-opening items — see the Monsoon window paragraph below)*: **T CrB observation restart at the October re-opening** (dark since 2025-06-24, 14+ months into Schaefer's window; the flickering science exists only if frames start flowing the night the dome re-opens — weekly flickering runs resume at re-opening for the brief evening-twilight window, pause through solar conjunction, and accrue mainly when the field re-emerges in the 2027 season, per the T CrB strategy's P0-1 visibility note; ≥6-run threshold unchanged) and the **consolidated calibration-frame acquisition request, now era-resolved from the S0b inventory** (`ops/2026-08_observatory_request.md` Item B reproduces the ranked `calib_gaps` shopping list — Mode0/era-76 biases+darks incl. the 240 s T CrB set, current Fast-mode ("era-C") bias/flat sets, the era-B-camera-survival question; ST LMi g/r/i season Oct 2026 → spring 2027 for the queue).

### Monsoon window (Aug–Oct 2026) — what proceeds, what waits *(2026-08-17 update)*
Winer is closed for monsoon season until October 2026; no new RLMT frames arrive before then. **Proceeds now (everything archival):** S1 astrometry, S2 detector/calibration work from archival ladders and the 2023 dark repeats, all external-data fetches (§3.3 — including the T CrB monsoon-gap coverage: AAVSO/ASAS-SN/ZTF/ATLAS photometry + ARAS spectra, fetched now, not in October), and all five papers' reductions and analyses on archival data. **Waits for October:** new calibration acquisitions (the era-resolved Item-B list), T CrB nightly/weekly frames, and the ST LMi g/r/i season. The observatory request is submitted now so the program is **first in queue at re-opening**; the eruption-response pyscope block is loaded and verified *during* the closure (software-side, no dome needed). If T CrB erupts during the monsoon, the eruption letter pivots to external data and RLMT joins post-restart. October observations are **first-class planned inputs** to the pipeline: each post-October archive sync is followed by an idempotent S0 + S0b re-run and a manifest diff (§6).
...
| R3 | **Missing mode-matched calibrations** (zero Mode0 darks/biases — under all 247 T CrB spectra; no StackPro/LowGain flats ever; no slot-'6' flat; no grism-era calib at all) | T CrB, BeStar, Dwarf | One consolidated acquisition request to Winer ops (chair signs, Week 1); archival frames get measured cross-mode penalty terms + the flanking-band adequacy test; delta-sky flats where dithered | Request submitted 2026-08-17; products at the October re-opening (no calibration frame can exist before the dome opens) |
...
| R8 | **Scooping / time-criticality** (T CrB eruption; Munari-adjacent groups; ELVES/HSC classifications; Labadie-Bartz Be program) | T CrB, Dwarf, BeStar | Consolidated **weekly sweep** (one person, one hour: arXiv + ATels + AAVSO alerts for all five target lists); T CrB restart at the October re-opening, first in queue; eruption-response pyscope block loaded during the closure (verified by 2026-09-01); verify-at-submission flags on all target-status claims | Standing |
...
### Week 1 (Aug 17–21)
- **D1 — Ship the consolidated observatory request** (chair; half a day, highest leverage in the portfolio) *(2026-08-17 update: Winer offline until October — the request is submitted now to execute at the October re-opening, first in queue)*: (a) restart T CrB at re-opening — nightly 3×1 s r + short B, one lrg + one hrg 240 s, weekly ≥2 hr B run at ≤60 s cadence (resuming at re-opening; accrues mainly in the 2027 season per the strategy's P0-1 visibility note); load the eruption-response pyscope block during the closure, verified before re-opening; (b) calibration acquisitions — the **era-resolved S0b `calib_gaps` shopping list** (Mode0/era-76 biases + darks incl. 240 s, Fast-mode/era-78/80 bias + flat g, ranked by science frames blocked) replaces the generic list; the era-B-camera-survival question stands; (c) queue request: one more g/r/i ST LMi season (October 2026 → spring 2027).
...
**Sprint exit criteria:** manifest is the single source of counts; astrometry path decided; detector memo v1 adopted; filter dossier v1 (broadband IDs + slot-'6') issued; all five projects' venue/scope branch points resolved or explicitly scheduled; observatory request **submitted and queued for the October re-opening** *(2026-08-17 update: replaces "T CrB frames flowing" — no frames are possible before October; escalate to the consortium board only if the queue slot is refused)*.

---

## 6. Standing cadence

- **Weekly:** consolidated literature/alert sweep (R8); T CrB restart status — through the monsoon closure this means queue-position + eruption-block-loaded status, then frame flow once October observing starts *(2026-08-17 update)*; sprint-board triage.
- **After each archive sync (post-October re-opening):** the **new-night ingest loop** *(2026-08-17 update)* — re-run S0 (manifest) then S0b (calibration inventory + raw↔reduced linkage), both idempotent, and diff the manifest against the previous build (frame/night/era counts, new-era detection, calib_gaps deltas). October observations are first-class planned inputs; this loop is how they enter every project's tables.
