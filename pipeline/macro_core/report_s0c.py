...
#: Display order and colors for staging roles (science leads; masters pale).
ROLE_ORDER = ("science", "bias", "dark", "flat",
              "master_bias", "master_dark", "master_flat")
ROLE_COLORS = {"science": ACCENT, "bias": "#9fd8ae", "dark": "#7a8b99",
...
def section_staged(con) -> str:
    ...
    for project, tbl in pairs:
        ...
        rows.append([esc(project), fmt(n_sci), fmt(n_tgt), fmt(n_nights),
                     f"{esc(first_n)} &rarr; {esc(last_n)}", fmt(n_eras),
                     fmt(n_cal), f"<code>{esc(csv_path)}</code>"])
    stage_tbl = table(
        ["project", "science", "targets", "nights", "span", "eras",
         "calib rows", "working artifact"], rows)
...
def section_calib(con) -> str:
    ...
    for project, tbl in pairs:
        by = dict(q(con, f"""SELECT role, count(*) FROM {tbl}
                             WHERE role != 'science' GROUP BY role"""))
    ...
    n_total_cal = sum(
        q1(con, f"SELECT count(*) FROM {tbl} WHERE role != 'science'")
        for _, tbl in pairs)
...
def section_provenance(con) -> str:
    ...
    file_rows = q(con, """
        SELECT project, stage_table, n_science, n_calib, n_rows, csv_path,
               n_symlinks
        FROM s0c_stage_files ORDER BY project""")
    files_tbl = table(
        ["project", "DB table", "science", "calib", "rows",
         "CSV (gitignored, regenerable)", "farm links"],
        [[esc(p), f"<code>{esc(t)}</code>", fmt(s), fmt(c), fmt(n),
          f"<code>{esc(f)}</code>", fmt(l) if l else "&mdash;"]
         for p, t, s, c, n, f, l in file_rows])
...
<div class="bhead"><h2>4 &middot; The provenance chain</h2>
...
<code>{esc(meta.get('git_commit', '') or 'uncommitted')}</code>):</p>
{files_tbl}
...
def render_report(manifest_path: Path) -> Path:
    ...
    try:
        ...
        n_sci = sum(q1(con, f"SELECT count(*) FROM {t} "
                            "WHERE role = 'science'") for _, t in pairs)
...
<nav>
  ...
  <a href="#provenance">4 Provenance chain</a>
</nav>
